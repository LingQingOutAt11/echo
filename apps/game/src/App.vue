...
const tarotBack = (card: Card) => {
  const hash = [...card.id].reduce(
    (sum, character) => sum * 31 + character.charCodeAt(0),
    0,
  );
  return `/tarot-cards/${TAROT_BACK_IDS[Math.abs(hash) % TAROT_BACK_IDS.length]}.webp`;
};
const destinyBack = (coverCard?: string) =>
  `/tarot-cards/${TAROT_BACK_IDS.includes(coverCard as (typeof TAROT_BACK_IDS)[number]) ? coverCard : TAROT_BACK_IDS[0]}.webp`;
const preloadTarotBacks = () =>
  TAROT_BACK_IDS.forEach((id) => {
...
const answeredStorageKey = (id: number | null) =>
  `ai-chemistry-answered-cards-${id}`;
const persistAnsweredCards = () =>
  localStorage.setItem(
    answeredStorageKey(userId.value),
    JSON.stringify(answeredCardIds.value),
  );
function drawDeck() {
...
function handleTarotTouchStart(event: TouchEvent) {
  tarotTouchStartX = event.touches[0]?.clientX ?? 0;
}
...
const userId = ref<number | null>(
  Number(localStorage.getItem("ai-chemistry-user-id")) || null,
);
...
const qrDataUrl = ref("");
const proximityId = ref(
  new URLSearchParams(window.location.search).get("proximity") ?? "",
);
...
const deviceId = ref(
  localStorage.getItem("ai-chemistry-device-id") ?? safeUUID(),
);
const round = ref(0);
const roundChoice = ref("");
const roundScores = ref<number[]>([]);
type DestinyState = {
  phase: "question" | "draw" | "card_pending" | "revealed";
  questionKey?: DestinyQuestionKey;
  selectedBy?: number;
  confirmedBy?: number;
  deck: DestinyCardKey[];
  cardKey?: DestinyCardKey;
  selectedCardBy?: number;
  confirmedCardBy?: number;
  comboKey?: string;
  comboName?: string;
  reading?: { prophecy: string; quote: string; opener: string };
  coverCard?: string;
};
...
const answeredCount = computed(
  () =>
    sessionCards.value.filter((card) => {
      const picked =
        answers.value[card.id]?.split(",").filter(Boolean).length ?? 0;
...
function startAutoRotate() {
  ...
  const tick = (now: number) => {
    ...
    if (!tarotFlipped.value) {
      ...
      const nearestIndex =
        Math.round(normalized / stepDeg) % Math.max(1, deck.value.length);
...
const selectedMatch = computed(
  () =>
    matches.value.find(
      (item) => Number(item.user.id) === Number(selectedMatchId.value),
    ) ?? null,
);
const demoMatches = ref(false);
const selectedReport = computed(
  () =>
    selectedMatch.value?.report ??
    chemistry(
      dna.value.dimensions,
      dna.value.dimensions,
      dna.value.tags,
      dna.value.tags,
      dna.value.dealBreakers,
      dna.value.dealBreakers,
      animal.value,
      animal.value,
    ),
...
const selectedAnimalCombo = computed(
  () =>
    selectedMatch.value?.report.combo ??
    animalCombo(animal.value, selectedMatch.value?.user.animal),
);
const pageTitles: Record<string, { step: string; label: string }> = {
  auth: { step: "00", label: "登录 / 注册" },
  profile: { step: "01", label: "回声档案" },
  account: { step: "00", label: "个人中心" },
  cards: { step: "02", label: "心智卡牌" },
  dna: { step: "03", label: "社交基因" },
  matches: { step: "04", label: "火花雷达" },
  detail: { step: "04", label: "化学解析" },
  dm: { step: "04", label: "在线聊天" },
  connect: { step: "05", label: "线下接触" },
  duo: { step: "06", label: "破冰对决" },
  destiny: { step: "06", label: "回声牌" },
  result: { step: "06", label: "回声结论" },
  chat: { step: "04", label: "内在小孩对话" },
};
const destinyQuestionCurrent = computed(() =>
  destinyQuestion(destiny.value?.questionKey),
);
const destinyCardCurrent = computed(() => destinyCard(destiny.value?.cardKey));
const destinySelectedByMe = computed(
  () => destiny.value?.selectedBy === userId.value,
);
const destinyCardSelectedByMe = computed(
  () => destiny.value?.selectedCardBy === userId.value,
);

const roundData = [
  {
    type: "AI 预测",
    question: "旅行时，谁会主动做攻略？",
    options: ["我", "TA"],
    tag: "PREDICTION",
  },
  {
    type: "默契互猜",
    question: "如果明天突然放假，TA 会选择？",
    options: ["去探索新地方", "在家躺平", "找朋友见面", "做自己的事"],
    tag: "GUESS",
  },
  {
    type: "分歧挑战",
    question: "30 秒内决定：第一次约会谁定餐厅？",
    options: ["我来定", "TA 来定", "一起决定", "交给随机"],
    tag: "CHALLENGE",
  },
];
    ...
    ? history.value.messages
        .filter(
          (message) =>
            ...
            Number(message.receiver_id) === partnerId,
        )
...
async function syncAnswered(userId: number) {
  ...
  try {
    local = JSON.parse(
      localStorage.getItem(answeredStorageKey(userId)) ?? "[]",
    ) as string[];
  } catch {
    /* 缓存损坏时忽略 */
  }
  ...
  if (!response.ok) {
    answeredCardIds.value = local;
    return;
  }
  const data = (await response.json()) as { answeredCardIds?: string[] };
  answeredCardIds.value = [
    ...new Set([...local, ...(data.answeredCardIds ?? [])]),
  ];
...
async function loadAccount() {
  ...
  try {
    ...
    if (!response.ok) {
      authToken.value = "";
      userId.value = null;
      localStorage.removeItem("ai-chemistry-auth-token");
      localStorage.removeItem("ai-chemistry-user-id");
      return false;
    }
    ...
    profile.value = {
      ...
      birth_datetime: data.user.birth_datetime ?? "",
      gender: data.user.gender === "male" ? "male" : "female",
      zodiac: data.user.zodiac ?? "",
      mbti: data.user.mbti ?? "",
      city: data.user.city,
      job: data.user.job,
      purpose: data.user.purpose as "恋爱" | "朋友" | "搭子",
      bio: data.user.bio,
    };
...
async function submitAuth() {
  ...
  if (authMode.value === "register") {
    if (
      authForm.value.username.trim().length < 2 ||
      /\s/.test(authForm.value.username)
    ) {
      error.value = "用户名需 2-40 位且不能包含空格";
      return;
    }
  ...
  try {
    const response = await apiFetch(`/auth/${authMode.value}`, {
      method: "POST",
        ...
        authMode.value === "register"
          ? data.error === "username_taken"
            ...
            : data.error === "VALIDATION"
              ? "注册信息有误：用户名 2-40 位（不含空格），密码至少 6 位"
              : "注册失败，请稍后重试"
    ...
    currentUser.value = data.user;
    userId.value = data.user.id;
    localStorage.setItem("ai-chemistry-auth-token", data.token);
    localStorage.setItem("ai-chemistry-user-id", String(data.user.id));
    await syncAnswered(data.user.id);
    await loadHistory();
...
async function refreshDmHistory() {
  ...
  const messages = new Map(
    [...history.value.messages, ...next.messages].map((message) => [
      message.id,
      message,
    ]),
  );
...
async function sendDm() {
  ...
  try {
    const response = await apiFetch("/messages", {
      method: "POST",
      body: JSON.stringify({
        receiverId: Number(selectedMatch.value.user.id),
        content: dmDraft.value.trim(),
      }),
    });
    if (response.ok) {
      ...
      history.value = {
        ...history.value,
        messages: [sent, ...history.value.messages],
      };
      dmDraft.value = "";
      await refreshDmHistory();
    } else if (response.status === 401)
      dmError.value = "登录已失效，请重新登录后发送。";
    else if (response.status === 404) dmError.value = "该匹配用户已不可用。";
    else dmError.value = `消息发送失败（${response.status}），请重试。`;
  } catch {
    dmError.value = "网络异常，消息未发送。";
  } finally {
    dmSending.value = false;
  }
}
watch(page, (current) => {
  if (current !== "dm") stopDmPolling();
});
const chemistryResult = computed(() => ({
  rapport: Math.round(
    roundScores.value.reduce((sum, score) => sum + score, 0) /
      Math.max(1, roundScores.value.length) || 82,
  ),
...
async function prepareQr() {
  qrDataUrl.value = await QRCode.toDataURL(
    `${window.location.origin}/?proximity=${encodeURIComponent(deviceId.value)}`,
    { width: 240, margin: 1, color: { dark: "#0b1426", light: "#f4efe3" } },
  );
...
watch(page, (current) => {
  if (current === "connect") {
    if (connectMethod.value === "qr") void prepareQr();
    startProximityPolling();
...
function swipeTarot(direction: -1 | 1) {
  cardIndex.value =
    (cardIndex.value + direction + Math.max(1, deck.value.length)) %
    Math.max(1, deck.value.length);
  const stepDeg = 360 / Math.max(1, deck.value.length);
...
async function answer(label: string) {
  ...
  if (card.multi) {
    selected.has(label)
      ...
      : selected.size < 3 && selected.add(label);
...
async function confirmMultiAnswer() {
  if (
    (answers.value[currentCard.value.id]?.split(",").filter(Boolean).length ??
      0) !== 3
  )
    return;
  await settleCard();
...
async function settleCard() {
  ...
  answeredCardIds.value = [
    ...new Set([...answeredCardIds.value, answeredCard.id]),
  ];
  persistAnsweredCards();
  if (answeredCount.value >= QUIZ_DRAW) {
    await submitAnswers();
    return;
  }
  ...
  let candidates = CARDS.filter(
    (card) =>
      !answeredCardIds.value.includes(card.id) && !activeIds.has(card.id),
  );
  if (!candidates.length) {
    ...
    candidates = CARDS.filter(
      (card) => !sessionIds.has(card.id) && !activeIds.has(card.id),
    );
  }
...
async function submitAnswers() {
  ...
  error.value = "";
  const payload = {
    answers: Object.entries(answers.value).flatMap(([cardId, labels]) =>
      labels
        .split(",")
        .filter(Boolean)
        .map((optionLabel) => ({ cardId, optionLabel })),
    ),
    dimensions: dna.value.dimensions,
    animal: animal.value,
    tags: dna.value.tags,
    dealBreakers: dna.value.dealBreakers,
  };
  try {
    const response = await apiFetch(`/users/${userId.value}/answers`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
    if (!response.ok) {
      error.value = "测评保存失败，请重新登录后重试。";
      return;
    }
    currentUser.value = {
      ...
      animal: animal.value,
      tags: dna.value.tags,
      deal_breakers: dna.value.dealBreakers,
    };
    const matchResponse = await apiFetch(`/users/${userId.value}/matches`);
    matches.value = matchResponse.ok
      ? ((await matchResponse.json()) as Match[])
      : [];
    await loadHistory();
...
function populateDemoMatches() {
  ...
  const makeUser = (
    ...
    tags: [],
    deal_breakers: [],
    animal: assignAnimal(dimensions),
  });
  const names = [
    "林澈",
    "安然",
  ...
  const users = ANIMALS.map((item, index) =>
    makeUser(
      101 + index,
      names[index] ?? `伙伴${index + 1}`,
      21 + (index % 12),
      item.vector,
    ),
  );
  matches.value = users.map((user) => {
    const report = chemistry(
      dna.value.dimensions,
      user.dimensions,
      dna.value.tags,
      user.tags,
      dna.value.deal_breakers ?? [],
      animal.value,
      user.animal,
    );
    // 推荐分随机分布在 80-98 之间
...
async function startProximityPolling() {
  ...
  proximityTimer = setInterval(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/proximity/announce`, {
        method: "POST",
        headers: { "content-type": "application/json", ...authHeaders() },
        body: JSON.stringify({
          deviceId: deviceId.value,
          targetDeviceId: proximityId.value || undefined,
        }),
      });
      if (!response.ok) {
        if (response.status === 401)
          proximityError.value = "登录已失效，请重新登录后再扫码配对。";
        else if (response.status === 422)
          proximityError.value = "二维码已失效，请让出码方重新生成。";
        else
          proximityError.value = `二维码配对服务暂时不可用（${response.status}），请稍后重试。`;
        return;
      }
      const data = (await response.json()) as {
        nearby?: boolean;
...
async function startDestiny() {
  if (!sessionId.value || sessionId.value.startsWith("mock-")) {
    destinyError.value = "需要两台已完成画像的手机扫码配对后，才能开启回声牌。";
    return;
  }
  destinyError.value = "";
  const response = await apiFetch(
    `/dual-sessions/${sessionId.value}/destiny/start`,
    { method: "POST" },
  );
  if (!response.ok) {
    destinyError.value = "回声牌会话未准备好，请让双方重新扫码。";
    return;
  }
  destiny.value = (await response.json()) as DestinyState;
  page.value = "destiny";
...
async function destinyAction(
  action:
    "select_question" | "confirm_question" | "select_card" | "confirm_card",
  key?: string,
) {
  if (!sessionId.value) return;
  const response = await apiFetch(
    `/dual-sessions/${sessionId.value}/destiny/action`,
...
async function nextRound() {
  if (!roundChoice.value) return;
  const optionIndex = currentRound.value.options.findIndex(
    (option) => option.label === roundChoice.value,
  );
  ...
  if (sessionId.value && !sessionId.value.startsWith("mock-")) {
    try {
      await apiFetch(`/dual-sessions/${sessionId.value}`, {
        method: "PATCH",
        body: JSON.stringify({
          rounds: roundScores.value,
          result: chemistryResult.value,
        }),
      });
    } catch {
...
function startCards() {
  error.value = "";
  if (!authToken.value || !userId.value) {
    page.value = "auth";
    return;
  }
  drawDeck();
...
function openChat() {
  ...
  if (!chatMessages.value.length)
    chatMessages.value = [{ role: "bot", text: "我也就是你呀。" }];
  chatError.value = "";
  chatQueued.value = "";
  page.value = "chat";
}
...
function persistChat() {
  try {
    localStorage.setItem(
      chatStorageKey(),
      JSON.stringify(chatMessages.value.slice(-40)),
    );
  } catch {
    /* 存储不可用时忽略 */
  }
...
async function sendChat() {
  ...
  try {
    const response = await apiFetch("/companion/chat", {
      ...
      body: JSON.stringify({
        query,
        user_id: String(userId.value ?? "guest"),
        system_prompt: chatPersona(),
      }),
      ...
      while ((sep = buffer.indexOf("\n\n")) >= 0) {
        ...
        try {
          ...
          const text =
            ...
            payload.text ??
            payload.message ??
            (payload.delta && typeof payload.delta === "string"
              ? payload.delta
              : undefined);
          if (event === "delta" && text) {
    ...
    if (!reply.text && buffer.trim()) {
      try {
        ...
        /* 非 JSON 文本直接展示 */ if (buffer.trim())
          reply.text = buffer.trim();
      }
    }
    if (!reply.text) reply.text = "(没有收到回复)";
  } catch (err) {
    if ((err as Error).name !== "AbortError")
      chatError.value = (err as Error).message || "对话失败,请重试";
  } finally {
...
<template>
  <main ref="stage" class="stage">
    ...
    <div class="shell">
      ...
      <header class="topbar">
        ...
        <div class="nav-tracker">
          ...
          <div class="step-indicator">
            ...
            ><span class="step-slash">/</span><span class="step-total">06</span>
          </div>
          <button
            v-if="authToken"
          ...
          <button v-if="authToken" class="account-nav-btn" @click="logout">
            退出
          </button>
        </div>
      </header>

      <section v-if="page === 'auth'" class="page-content auth-stage">
        <div class="stage-header text-center">
          <div class="pill-badge centered">SOCIAL DNA ACCESS</div>
          <h1 class="section-title">先登录，<em>再认识真正的自己。</em></h1>
          <p class="section-lead">
            注册后你的动物塑、性格光谱和沟通记录都会保存在个人中心。
          </p>
        </div>
        ...
        <form class="glass-panel auth-card" @submit.prevent="submitAuth">
          <div class="input-group">
            <label
              ><span class="label-txt">用户名 / USERNAME</span
              ><input
          ...
          <div class="input-group">
            <label
              ><span class="label-txt">密码 / PASSWORD</span
              ><input
          ...
          <template v-if="authMode === 'register'"
            ><div class="form-grid auth-register-grid">
              <div class="input-group">
                <label
                  ><span class="label-txt">昵称 / NAME</span
                  ><input v-model.trim="authForm.nickname" required
                /></label>
              </div>
              <div class="input-group">
                <label
                  ><span class="label-txt">出生日期时间 / BIRTH</span
                  ><input
                    v-model="authForm.birth_datetime"
                    type="datetime-local"
                    required
                    @click="openPicker"
                /></label>
              ...
              <div class="input-group">
                <label
                  ><span class="label-txt">性别 / GENDER</span
                  ><select v-model="authForm.gender" required>
              ...
              <div class="input-group">
                <label
                  ><span class="label-txt">星座 / ZODIAC</span
                  ><select v-model="authForm.zodiac" required>
                    <option value="" disabled>选择星座</option>
                    <option v-for="item in ZODIACS" :key="item" :value="item">
                      {{ item }}
                    </option>
                  </select></label
                >
              ...
              <div class="input-group">
                <label
                  ><span class="label-txt">MBTI</span
                  ><select v-model="authForm.mbti" required>
                    <option value="" disabled>选择 MBTI</option>
                    <option v-for="item in MBTIS" :key="item" :value="item">
                      {{ item }}
                    </option>
                  </select></label
                >
              ...
              <div class="input-group">
                <label
                  ><span class="label-txt">城市 / CITY</span
                  ><input v-model.trim="authForm.city" required
                /></label>
              </div>
              <div class="input-group">
                <label
                  ><span class="label-txt">职业 / OCCUPATION</span
                  ><input v-model.trim="authForm.job" required
                /></label>
              </div>
              <div class="input-group">
                <label
                  ><span class="label-txt">来意 / PURPOSE</span
                  ><select v-model="authForm.purpose" required>
      ...
      <section
        v-else-if="page === 'account'"
        class="page-content account-stage"
      >
        <div class="stage-header">
          <div class="pill-badge">MY CHEMISTRY PROFILE</div>
          <h1 class="section-title">
            {{ currentUser?.nickname }} 的<em>动物塑档案。</em>
          </h1>
          <p class="section-lead">
            保存你的动物塑图片、12 维性格光谱和沟通记录。
          </p>
        </div>
        <article v-if="currentUser?.animal" class="account-animal-card">
          <img
            :src="currentUser.animal.image"
            :alt="currentUser.animal.name"
            class="account-animal-image"
          />
          <div class="account-animal-copy">
            <span class="totem-tag">YOUR SOCIAL DNA</span>
            <h2>
              {{ currentUser.animal.name }} · {{ currentUser.animal.title }}
            </h2>
            <p>{{ currentUser.animal.tagline }}</p>
          </div>
          <button class="primary-btn account-radar-btn" @click="openMatches">
            <span class="btn-text">进入火花雷达</span>
          </button>
          <button class="primary-btn account-chat-btn" @click="openChat">
            <span class="btn-text">和内在小孩聊聊 💬</span>
          </button>
        </article>
        <article v-else class="glass-panel account-empty">
          <h2>还没有动物塑结果</h2>
          <p>完成心智卡牌后生成专属卡片。</p>
          <button class="primary-btn" @click="page = 'profile'">
            <span class="btn-text">开始测算</span
            ><span class="btn-arrow-box">→</span>
          </button>
        </article>
        <div v-if="currentUser?.dimensions" class="account-dimensions">
          <div v-for="item in DIMENSIONS" :key="item.id" class="dim-card">
            <div class="dim-info">
              <span class="dim-name">{{ item.group }} · {{ item.label }}</span
              ><span class="dim-score">{{
                currentUser.dimensions[item.id]
              }}</span>
            </div>
            <div class="dim-bar-track">
              <div
                class="dim-bar-fill"
                :style="{ width: `${currentUser.dimensions[item.id]}%` }"
              />
            </div>
          </div>
        </div>
        <div class="account-columns">
          <article class="glass-panel history-card">
            <span class="panel-tag">ICEBREAKER HISTORY</span>
            <p
              v-for="session in history.sessions"
              :key="session.id"
              class="history-item"
            >
              {{ session.partner_nickname ?? "匹配伙伴" }} ·
              {{ session.status }}
            </p>
            <p v-if="!history.sessions.length" class="empty-history">
              还没有破冰会话记录。
            </p>
          </article>
        </div>
      ...
      <section v-if="page === 'profile'" class="page-content hero-page">
        <div class="hero-header">
          ...
          <p class="lead-text">
            AI
            不替你社交，它只通过真实的本能抉择，为两位本该相遇的灵魂架起一座破冰桥。
          </p>
        </div>
          ...
          <div class="form-grid">
            ...
            <div class="input-group">
              <label
                ><span class="label-txt">出生日期时间 / BIRTH</span
                ><input
                  v-model="profile.birth_datetime"
                  type="datetime-local"
                  @click="openPicker"
              /></label>
            </div>
            ...
            <div class="input-group">
              <label
                ><span class="label-txt">城市 / CITY</span
                ><input v-model="profile.city" placeholder="上海"
              /></label>
            </div>
            <div class="input-group">
              <label
                ><span class="label-txt">职业 / OCCUPATION</span
                ><input v-model="profile.job" placeholder="产品设计师 / 建筑师"
              /></label>
            </div>
            <div class="input-group full-width">
              <label>
                ...
                <div class="segmented-control">
                  <button
                    type="button"
                    :class="{ active: profile.purpose === '恋爱' }"
                    @click="profile.purpose = '恋爱'"
                  >
                    <svg
                      viewBox="0 0 20 20"
                      fill="currentColor"
                  ...
                  <button
                    type="button"
                    :class="{ active: profile.purpose === '朋友' }"
                    @click="profile.purpose = '朋友'"
                  >
                    <svg
                      viewBox="0 0 20 20"
                      fill="currentColor"
                  ...
                  <button
                    type="button"
                    :class="{ active: profile.purpose === '搭子' }"
                    @click="profile.purpose = '搭子'"
                  >
                    <svg
                      viewBox="0 0 20 20"
                      fill="currentColor"
            ...
            <div class="input-group full-width">
              <label>
                ...
                <textarea
                  v-model="profile.bio"
                  maxlength="200"
                  placeholder="分享一句你的真实状态：最近在为什么事物着迷？或者周末最常出没的角落？"
                  rows="3"
                />
              </label>
            </div>
      ...
      <section
        ...
        class="page-content cards-stage tarot-stage"
      >
        <div v-if="error" class="error-banner">{{ error }}</div>
        <div class="quiz-nav">
          <div class="quiz-meta">
            <span class="badge-cat">TAROT DRAW</span
            ><span class="quiz-id"
              >题库 {{ POOL_SIZE }} · {{ progress }} 已答</span
            >
          </div>
          <div class="quiz-counter">
            <span class="current-step">{{ answeredCount }}</span
            ><span class="slash">/</span
            ><span class="max-step">{{ QUIZ_DRAW }}</span>
          </div>
        </div>
        <div class="quiz-progress-track">
          <div
            class="quiz-progress-bar"
            :style="{ width: `${(answeredCount / QUIZ_DRAW) * 100}%` }"
          />
        </div>
        <div class="tarot-intro">
          <span class="tarot-kicker">THE SOCIAL ARCANA</span>
          <h1 class="tarot-title">抽一张牌，<em>看见你的第一反应。</em></h1>
          <p>
            从 {{ POOL_SIZE }} 道题库中随机抽出 {{ QUIZ_DRAW }} 张。8
            张全部作答后，自动进入社交基因解码。
          </p>
        </div>
        <div
          class="tarot-carousel"
          aria-label="本轮八张塔罗情境牌"
          @touchstart="handleTarotTouchStart"
          @touchend="handleTarotTouchEnd"
        >
          <button
            ...
            class="tarot-card"
            :class="{
              flipped: tarotFlipped && index === cardIndex,
              answered: answers[card.id],
            }"
            :style="tarotTransform(index)"
            :aria-label="
              index === cardIndex && tarotFlipped
                ? `回答${card.title}`
                : '抽取这张塔罗牌'
            "
            @click="focusTarot(index)"
          >
            <span class="tarot-card-inner"
              ><span class="tarot-face tarot-back"
                ><img
                  :src="tarotBack(card)"
                  :alt="`${card.category}塔罗牌背`" /></span
              ><span class="tarot-face tarot-front"
                ><span class="tarot-card-index"
                  >{{ String(index + 1).padStart(2, "0") }} ·
                  {{ card.category }}</span
                ><strong>{{ card.title }}</strong
                ><small>{{ card.description }}</small
                ><span class="tarot-flip-hint">{{
                  card.multi ? "选择 3 项" : "选择你的答案"
                }}</span></span
              ></span
            >
          </button>
        </div>
        <article v-if="tarotFlipped" class="tarot-answer-panel">
          <div class="tarot-answer-heading">
            <span>{{ currentCard.category }} · {{ currentCard.id }}</span
          ...
          <div class="options-container">
            <button
              ...
              @click.stop="answer(option.label)"
            >
              <span class="option-key">{{ option.label }}</span
              ><span class="option-text">{{ option.text }}</span
              ><span class="option-select-glow">✓</span>
            </button>
          </div>
          <button
            v-if="currentCard.multi"
            class="primary-btn multi-confirm"
            :disabled="
              answers[currentCard.id]?.split(',').filter(Boolean).length !== 3
            "
            @click="confirmMultiAnswer"
          >
            <span class="btn-text">确认 3 项红线</span
            ><span
              >{{
                answers[currentCard.id]?.split(",").filter(Boolean).length ?? 0
              }}/3</span
            >
          </button>
        </article>
      ...
      <section v-else-if="page === 'dna'" class="page-content dna-stage">
        <div v-if="error" class="error-banner">{{ error }}</div>
        <div class="stage-header text-center">
          <div class="pill-badge centered">SOCIAL DNA DECODED</div>
          <h1 class="section-title">AI 眼中的你，<em>只会是一种动物。</em></h1>
          <p class="section-lead">你的选择已经转化为 12 维性格光谱。</p>
        </div>
        <div class="totem-hero-card">
          <div class="totem-pattern-grid" />
          <div class="totem-badge-top">
            <span class="totem-tag">YOUR ONE SOCIAL DNA</span
            ><span class="totem-mix-badge">唯一动物塑</span>
          </div>
          <div class="totem-core">
            <img
              class="totem-animal-card"
              :src="animal.image"
              :alt="animal.name"
            />
            <div class="totem-details">
              <h3 class="totem-name">{{ animal.name }} · {{ animal.title }}</h3>
              <p class="totem-desc">{{ animal.tagline }}</p>
            </div>
          </div>
        </div>
        <div class="dimensions-block">
          <h4 class="block-title">✦ 你的性格光谱 <small>Top 4</small></h4>
          <div class="dimension-bars-grid">
            <div v-for="item in highlights" :key="item.id" class="dim-card">
              <div class="dim-info">
        ...
        <blockquote class="soul-quote">
          ...
          <p>{{ insight(animal, dna.dimensions) }}</p>
        </blockquote>
        <details class="radar-details">
          <summary>查看完整 12 维雷达</summary>
          <div class="dimension-bars-grid">
            <div v-for="item in DIMENSIONS" :key="item.id" class="dim-card">
              <div class="dim-info">
                <span class="dim-name">{{ item.group }} · {{ item.label }}</span
                ><span class="dim-score">{{ dna.dimensions[item.id] }}</span>
              </div>
              <div class="dim-bar-track">
                <div
                  class="dim-bar-fill"
                  :style="{ width: `${dna.dimensions[item.id]}%` }"
                />
              </div>
            </div>
          </div>
        ...
        <button class="primary-btn" @click="page = 'matches'">
          <span class="btn-text">看看 AI 为你找到的人</span>
          <div class="btn-arrow-box">→</div>
        </button>
        <button class="chat-entry-btn" @click="openChat">
          💬 和内在小孩聊聊 →
        </button>
      </section>
      ...
      <section
        ...
        class="page-content matches-stage"
      >
        <div class="stage-header">
          <div class="pill-badge">CHEMISTRY RADAR</div>
          <h1 class="section-title">值得认识的，<em>从来不需要一百个。</em></h1>
          <p class="section-lead">
            {{
              demoMatches
                ? "当前为演示匹配，邀请真实同伴完成测算后自动替换。"
                : "每天为你筛出最多 20 位可能有火花的人。"
            }}
          </p>
        </div>
        <div v-if="matches.length" class="matches-list-grid">
          ...
          >
            <div class="match-avatar-pill">
              <img
                v-if="item.user.animal?.image"
                :src="item.user.animal.image"
                :alt="item.user.animal.name"
            ...
            <div class="match-card-actions">
              <button
                ...
                :aria-label="`和 ${item.user.nickname} 聊天`"
                @click.stop="openDmFromCard(item.user.id)"
              >
                <svg viewBox="0 0 20 20" fill="currentColor">
                  <path
        ...
        <div v-else class="empty-glass-state">
          ...
          <h3>正在等待同频信号入网...</h3>
          <p>
            当前空间内尚未形成足够心智聚类。建议邀请同场伙伴完成测算，火花雷达将实时点亮。
          </p>
        </div>
      </section>
      ...
      <section v-else-if="page === 'detail'" class="page-content detail-stage">
        ...
        <div class="detail-header-card">
          <div class="avatar-large-box">
            <img
              v-if="selectedUser.animal?.image"
              :src="selectedUser.animal.image"
              :alt="selectedUser.animal.name"
              class="avatar-large-img"
            /><span v-else class="avatar-large-text">{{
          ...
          <div class="detail-user-main">
            ...
            <h2 class="detail-nickname">
              {{ selectedUser.nickname }} ·
              {{ selectedUser.animal?.name ?? "等待连接" }}
            </h2>
            <p class="detail-job-text">
              {{ selectedUser.job }} ·
              {{ selectedUser.animal?.title ?? "真实参与者" }}
            </p>
          </div>
        ...
        <div class="animal-meet-card">
          <span>{{ animal.emoji }} {{ animal.name }}</span
          ><strong>×</strong
          ><span
            >{{ selectedUser.animal?.emoji ?? "🧪" }}
            {{ selectedUser.animal?.name ?? "TA" }}</span
          ><em>{{ selectedAnimalCombo ?? "相遇组合" }}</em>
        </div>
        ...
        <div class="chemistry-cards-grid">
          <article class="chem-card chem-common">
            ...
            <div class="chem-card-body">
              <p
                v-for="text in selectedReport.common"
                :key="text"
                class="bullet-item"
              >
                <span class="bullet-dot" />{{ text }}
              </p>
              <p v-if="!selectedReport.common.length" class="bullet-item">
                在生活步调上保持着相近的审美与节奏感。
              </p>
            </div>
          </article>
          ...
          <article class="chem-card chem-complement">
            ...
            <div class="chem-card-body">
              <p
                v-for="text in selectedReport.complements"
                :key="text"
                class="bullet-item"
              >
                <span class="bullet-dot" />{{ text }}
              </p>
              <p v-if="!selectedReport.complements.length" class="bullet-item">
                你们的节奏有充裕的留白空间，能互相提供新鲜视角。
              </p>
            </div>
          </article>
        ...
        <div class="icebreaker-starter-box">
          <div class="starter-tag">
            <svg viewBox="0 0 20 20" fill="currentColor" class="starter-icon">
              <path
                d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z"
              />
              <path
        ...
        <div class="detail-action-row">
          <button class="primary-btn detail-chat-btn" @click="openDm">
            <svg viewBox="0 0 20 20" fill="currentColor" class="btn-icon-sm">
              <path
                d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z"
              />
              <path
      ...
      <section
        ...
        class="page-content connect-stage"
      >
        <div class="stage-header text-center">
          ...
          <h1 class="section-title">一个人生成，<em>一个人扫码。</em></h1>
          <p class="section-lead">
        ...
        <div class="interaction-terminal">
          <div class="terminal-corner tl" />
          <div class="terminal-corner tr" />
          <div class="terminal-corner bl" />
          <div class="terminal-corner br" />
          ...
          <template v-if="connectMethod === 'proximity'">
            ...
            <p v-if="proximityError" class="proximity-error-text">
              {{ proximityError }}
            </p>
            <button
              class="primary-btn proximity-exit-btn"
              @click="leaveProximity"
            >
              <span class="btn-text">退出等待</span>
            </button>
          </template>
          ...
          <template v-else>
            ...
            <p class="nfc-status-sub">
              对方扫码并登录后，将自动进入专属破冰对决，无需其他操作
            </p>
            <p v-if="proximityError" class="proximity-error-text">
              {{ proximityError }}
            </p>
      ...
      <section v-else-if="page === 'duo'" class="page-content duo-stage">
        ...
        <article class="interactive-card duo-card">
          ...
          <p class="duo-prompt-hint">
            双方各自在手机上做出抉择，倒计时结束共同揭晓。
          </p>
          ...
          <div class="duo-options-grid">
            ...
            >
              ...
              <div class="duo-state-indicator">
                <span v-if="roundChoice === option" class="state-selected"
                  >已选定</span
                >
      ...
      <section
        v-else-if="page === 'destiny'"
        class="page-content destiny-stage"
      >
        ...
        <template v-if="destiny?.phase === 'question'">
          <div class="destiny-status">
            {{
              destinyQuestionCurrent
                ? destinySelectedByMe
                  ? "已发送给对方确认"
                  : "对方想问这一题，等你说“我也想知道”"
                : "从五个未来里，选一个你们都想知道的答案"
          ...
          <div class="destiny-question-grid">
            <button
              v-for="question in DESTINY_QUESTIONS"
              :key="question.key"
              class="destiny-question"
              :class="{ selected: destiny?.questionKey === question.key }"
              :disabled="Boolean(destiny?.questionKey)"
              @click="destinyAction('select_question', question.key)"
            >
              <span>{{ question.temperature }}</span
              ><strong>{{ question.label }}</strong>
            </button>
          </div>
          <button
            v-if="destinyQuestionCurrent && !destinySelectedByMe"
            class="primary-btn"
            @click="destinyAction('confirm_question')"
          >
            <span class="btn-text">我也想知道</span
            ><span class="btn-arrow-box">→</span>
          </button>
        </template>
        ...
        <template
          v-else-if="
            destiny?.phase === 'draw' || destiny?.phase === 'card_pending'
          "
        >
          <p class="destiny-status">
            {{
              destiny?.phase === "card_pending"
                ? destinyCardSelectedByMe
                  ? "牌已选好，等对方确认翻开"
                  : "对方选中了这张牌，点击确认一起翻开"
                : "从十二张回声牌里，凭边缘颜色抽一张写给你们的镜头"
            }}
          </p>
          <div class="destiny-deck">
            <button
              v-for="key in destiny?.deck"
              :key="key"
              class="destiny-card"
              :class="{ pending: destiny?.cardKey === key }"
              :style="{ '--destiny-accent': destinyCard(key)?.accent }"
              :disabled="destiny?.phase !== 'draw'"
              @click="destinyAction('select_card', key)"
            >
              <span>回声牌</span><i>⌁</i
              ><small>{{ destiny?.cardKey === key ? "已选择" : "抽取" }}</small>
            </button>
          </div>
          <button
            v-if="destiny?.phase === 'card_pending' && !destinyCardSelectedByMe"
            class="primary-btn"
            @click="destinyAction('confirm_card')"
          >
            <span class="btn-text">一起翻开</span
            ><span class="btn-arrow-box">→</span>
          </button>
        </template>
        ...
        >
          <article
            class="destiny-reveal"
            :style="{ '--destiny-accent': destinyCardCurrent.accent }"
          >
            <div class="destiny-face">
              <img :src="destinyBack(destiny?.coverCard)" alt="回声牌卡背" />
            </div>
            <div class="destiny-copy">
              <span
                >{{ destiny?.comboName }} ·
                {{ destinyQuestionCurrent.label }}</span
              >
              <p>{{ destiny.reading.prophecy }}</p>
              <blockquote>{{ destiny.reading.quote }}</blockquote>
              <div class="destiny-opener">
                <b>现在，问问对方：</b>{{ destiny.reading.opener }}
              </div>
            </div>
          </article>
          <div class="destiny-actions">
            <button class="primary-btn" @click="continueDuo">
              <span class="btn-text">把话留给彼此，继续协作</span
              ><span class="btn-arrow-box">→</span></button
            ><button class="back-link-btn" @click="continueDuo">
              跳过，直接聊
            </button>
          </div>
        </template>
      ...
      <section v-else-if="page === 'result'" class="page-content result-stage">
        ...
        <div class="metrics-triad-grid">
          <div class="metric-card">
            ...
            <div class="metric-value">
              {{ chemistryResult.rapport }}<small>%</small>
            </div>
          ...
          <div class="metric-card featured">
            ...
            <div class="metric-value featured-value">
              {{ chemistryResult.spark }}<small>%</small>
            </div>
          ...
          <div class="metric-card">
            <div class="metric-icon-wrap">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <circle cx="12" cy="12" r="9" stroke-width="1.75" />
                <path d="M12 3v18M3 12h18" stroke-width="1.75" />
              </svg>
            </div>
            <div class="metric-value">
              {{ chemistryResult.complement }}<small>%</small>
            </div>
        ...
        <blockquote class="soul-quote final-quote">
          <div class="quote-symbol">“</div>
          <p>
            「你们不一定需要在所有观念上完全一致，但显而易见，你们非常值得在今晚继续聊下去。」
          </p>
        </blockquote>
      ...
      <section v-else-if="page === 'chat'" class="page-content chat-stage">
        <div class="chat-header">
          <img
            :src="chatAnimal.image"
            class="chat-avatar"
            :alt="chatAnimal.name"
          />
          <div class="chat-header-copy">
            <h1 class="chat-title">
              {{ chatAnimal.emoji }} {{ chatAnimal.name }} · 你的内在小孩
            </h1>
            <p class="chat-subtitle">
              {{ chatAnimal.tagline }} —— TA 就是你心里那个还没长大的自己。
            </p>
          </div>
        </div>
        ...
        <div class="chat-window">
          <div v-if="!chatMessages.length" class="chat-empty">
            <p>
              TA 正等着你开口。<br />聊聊今天的心情、最近的烦恼，或者随便什么都行。
            </p>
          </div>
          <div
          ...
          >
            ...
            <div class="chat-bubble">
              ...
              <details v-if="message.thinking" class="chat-thinking">
          ...
          <div v-if="chatStreaming" class="chat-typing">
      ...
      <section
        v-else-if="page === 'dm'"
        class="page-content chat-stage dm-stage"
      >
        <div class="chat-header">
          <img
            v-if="selectedUser.animal?.image"
            ...
            :alt="selectedUser.animal.name"
          />
          <div v-else class="chat-avatar dm-avatar-fallback">
            {{ selectedUser.nickname.slice(0, 1) }}
          </div>
          <div class="chat-header-copy">
            <h1 class="chat-title">
              {{ selectedUser.nickname }} ·
              {{ selectedUser.animal?.name ?? "匹配用户" }}
            </h1>
            <p class="chat-subtitle">
              {{ selectedUser.job }} · {{ selectedUser.city }} · 化学值
              {{ selectedReport.total }}%
            </p>
          </div>
          <button class="back-link-btn dm-back-btn" @click="page = 'detail'">
            <span>返回</span>
          </button>
        </div>
        ...
        <div class="chat-window">
          <div v-if="!selectedMessages.length" class="chat-empty">
            <p>
              你们还没有聊过天。<br />用推荐的第一句话开场：「{{
                selectedReport.firstMessage
              }}」
            </p>
          </div>
          <div
            ...
            :key="message.id"
            class="chat-msg"
            :class="Number(message.sender_id) === userId ? 'user' : 'peer'"
          >
            <img
              v-if="
                Number(message.sender_id) !== userId &&
                selectedUser.animal?.image
              "
              :src="selectedUser.animal.image"
              class="chat-bubble-avatar"
              :alt="selectedUser.animal.name"
        ...
        <div class="chat-input-row">
          <input
            v-model="dmDraft"
            class="chat-input"
            :placeholder="`发消息给 ${selectedUser.nickname}…`"
            maxlength="1000"
            :disabled="dmSending"
            @keyup.enter="sendDm"
          />
